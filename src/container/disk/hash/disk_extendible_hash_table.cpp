...
DiskExtendibleHashTable<K, V, KC>::DiskExtendibleHashTable(const std::string &name, BufferPoolManager *bpm,
  ...
  auto header_page = header_guard.AsMut<ExtendibleHTableDirectoryPage>();
...
auto DiskExtendibleHashTable<K, V, KC>::GetValue(const K &key, std::vector<V> *result, Transaction *transaction) const
  ...
  auto bucket_page_id = directory_page->GetBucketPageId(hash);
  LOG_DEBUG("the bucket_page_id is: %d, hash is %d", bucket_page_id, hash);
...
auto DiskExtendibleHashTable<K, V, KC>::Insert(const K &key, const V &value, Transaction *transaction) -> bool {
  ...
  bool key_exists = GetValue(key, &valuesfound, transaction);
  if (key_exists) {
